...
typedef void (^data_callback)(BRWebSocket *webSocket,  NSData *data);
...
@interface BRWebSocket ()  <NSStreamDelegate>
...
@implementation BRWebSocket {
    ...
    __strong BRWebSocket *_selfRetain;
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    //HOJJAT : pass 401 as error code to indicate an authorization problem
    if (responseCode == 401) {
        SRFastLog(@"Request failed with response code %d", responseCode);
        [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain code:401 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], SRHTTPResponseErrorKey:@(responseCode)}]];
        return;
    }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(BRWebSocket *self,  NSData *data) {
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(BRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(BRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(BRWebSocket *self, NSData *data) {
